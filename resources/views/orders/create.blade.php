        ...
        <form action="/orders" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('order_number') is-invalid @enderror" id="order_number"
                    name="order_number" required value="{{ old('order_number') }}" min="1" max="99999999999999999999"
                    autofocus>
                @error('order_number')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('supplier_name') is-invalid @enderror" id="supplier_name"
                    name="supplier_name" required value="{{ old('supplier_name') }}" maxlength="50">
                @error('supplier_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('product_name') is-invalid @enderror" id="product_name"
                    name="product_name" required value="{{ old('product_name') }}" maxlength="50">
                @error('product_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('total') is-invalid @enderror" id="total"
                    name="total" required value="{{ old('total') }}" min="1" max="99999999">
                @error('total')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <button type="submit" class="btn btn-primary mb-5">Create Order</button>
        </form>
    </div>
    <script>
